...
export default function Pipeline() {
  ...
  return (
    <InstallerLayout>
      <div className="mb-8">
        <h1 className="text-3xl font-bold text-black mb-2">Pipeline Lead</h1>
        <p className="text-black/70">Gestisci lo stato delle tue lead</p>
      </div>

      <div className="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-4 gap-4">
        {PIPELINE_STAGES.map((stage) => (
          <div key={stage.status} className="flex flex-col">
            <div className={`${stage.color} border-2 rounded-lg p-4 mb-4`}>
              <h3 className="font-bold text-black mb-1">{stage.label}</h3>
              <p className="text-sm text-black/70">{leadsByStatus[stage.status].length} lead</p>
            </div>
            ...
            <div className="space-y-3 flex-1">
              {leadsByStatus[stage.status].map((lead) => (
                <div
                  key={lead.id}
                  className="bg-white rounded-lg border border-cool-gray-300 p-4 hover:shadow-lg transition-all"
                >
                  <Link to={`/installer/leads/${lead.id}`}>
                    <h4 className="font-semibold text-black mb-2 hover:text-reflex-blue">
                  ...
                  <div className="space-y-1 mb-3">
                    {lead.phone && (
                      <div className="flex items-center gap-2 text-xs text-black/70">
                        ...
                        <a href={`tel:${lead.phone}`} className="hover:text-reflex-blue">
                          {lead.phone}
                        </a>
                      </div>
                    )}
                  ...
                  <select
                    ...
                    className="w-full text-xs px-2 py-1.5 border border-cool-gray-400 rounded bg-white hover:bg-cool-gray-100 transition-all cursor-pointer"
                  >
                  ...
                  <Link
                    ...
                    className="mt-2 flex items-center justify-center gap-1 text-xs text-reflex-blue-600 hover:text-reflex-blue font-medium"
                  >
                    Dettagli
              ...
              {leadsByStatus[stage.status].length === 0 && (
                <div className="bg-cool-gray-100 rounded-lg border-2 border-dashed border-cool-gray-300 p-4 text-center">
