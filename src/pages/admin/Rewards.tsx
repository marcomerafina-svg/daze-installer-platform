    ...
    <AdminLayout>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
        <div className="bg-white rounded-squircle border border-daze-gray p-6">
          ...
          <h3 className="text-daze-black/80 text-sm font-inter font-medium mb-1">Installatori Totali</h3>
          <p className="text-3xl font-bold text-daze-black mb-1">{stats.totalInstallers}</p>
          <p className="text-sm font-inter text-daze-black/60">{stats.activeInstallers} attivi</p>
        </div>

        <div className="bg-white rounded-squircle border border-daze-gray p-6">
          <div className="flex items-start justify-between mb-4">
            <div className="bg-daze-honey p-3 rounded-lg">
              <Award className="w-6 h-6 text-white" />
            </div>
          </div>
          <h3 className="text-daze-black/80 text-sm font-inter font-medium mb-1">Punti Totali</h3>
          <p className="text-3xl font-bold text-daze-black mb-1">{stats.totalPoints.toLocaleString('it-IT')}</p>
          <p className="text-sm font-inter text-daze-black/60">Sistema rewards</p>
        </div>

        <div className="bg-white rounded-squircle border border-daze-gray p-6">
          <div className="flex items-start justify-between mb-4">
            <div className="bg-daze-forest p-3 rounded-lg">
              <TrendingUp className="w-6 h-6 text-white" />
            </div>
          </div>
          <h3 className="text-daze-black/80 text-sm font-inter font-medium mb-1">Media Punti</h3>
          <p className="text-3xl font-bold text-daze-black mb-1">{stats.avgPoints.toLocaleString('it-IT')}</p>
          <p className="text-sm font-inter text-daze-black/60">Per installatore</p>
        </div>
        ...
        <div className="bg-white rounded-squircle border border-daze-gray p-6">
          ...
          <h3 className="text-daze-black/80 text-sm font-inter font-medium mb-1">Tier Attivi</h3>
          <p className="text-3xl font-bold text-daze-black mb-1">{tiers.length}</p>
          <p className="text-sm font-inter text-daze-black/60">Livelli disponibili</p>
        </div>
      </div>

      <div className="bg-white rounded-squircle border border-daze-gray p-6 mb-8">
        <h2 className="text-xl font-bold text-daze-black mb-4">Tier System</h2>
        <div className="grid grid-cols-1 md:grid-cols-5 gap-4">
          {tiers.map((tier) => {
            const installersInTier = leaderboard.filter((e) => e.current_tier?.id === tier.id).length;
            return (
              <div key={tier.id} className="text-center p-4 bg-daze-gray/10 rounded-xl font-inter">
                <TierBadge tier={tier} size="lg" showLabel={false} className="mb-2" />
                <p className="font-bold text-daze-black text-sm mb-1">{tier.display_name}</p>
                <p className="text-xs text-daze-black/70 mb-2">{tier.points_required.toLocaleString('it-IT')} pts</p>
                <p className="text-lg font-bold text-daze-blue">{installersInTier}</p>
                <p className="text-xs text-daze-black/60">installatori</p>
              </div>
            );
          })}
        </div>
        ...
        <div className="p-6 border-b border-daze-gray">
          <div className="relative">
            ...
            <input
              ...
              className="w-full pl-12 pr-4 py-3 border border-daze-gray rounded-lg outline-none focus:ring-0 focus:border-daze-blue transition-all"
            />
          </div>
        </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full">
            <thead className="bg-daze-gray/10 border-b border-daze-gray font-inter">
              <tr>
                <th className="text-center px-6 py-4 text-sm font-semibold text-daze-black w-20">Rank</th>
                <th className="text-left px-6 py-4 text-sm font-semibold text-daze-black">Installatore</th>
                <th className="text-center px-6 py-4 text-sm font-semibold text-daze-black">Tier</th>
                <th
                  className="text-center px-6 py-4 text-sm font-semibold text-daze-black cursor-pointer hover:bg-daze-gray/20 transition-colors"
            ...
            <tbody className="divide-y divide-daze-gray font-inter">
              {filteredLeaderboard.length === 0 ? (
                <tr>
                  <td colSpan={7} className="text-center py-12 text-daze-black/60">
                    Nessun installatore trovato
                  </td>
                </tr>
              ) : (
                filteredLeaderboard.map((entry, index) => (
                  <tr key={entry.id} className="hover:bg-daze-gray/10 transition-colors">
                    ...
                    <td className="px-6 py-4">
                      <div>
                        ...
                        <p className="text-sm text-daze-black/60">{entry.email}</p>
                      </div>
                    </td>
                    <td className="text-center px-6 py-4">
                    ...
                    <td className="text-center px-6 py-4">
                      <span className="inline-flex items-center justify-center px-4 py-2 bg-daze-blue-light rounded-lg font-bold text-daze-blue">
                        {entry.total_points.toLocaleString('it-IT')}
                      </span>
                    </td>
                    <td className="text-center px-6 py-4">
                      <span className="inline-flex items-center justify-center w-12 h-12 bg-daze-forest/10 rounded-full font-semibold text-daze-forest">
                        {entry.won_leads}
                      </span>
                    </td>
                    <td className="text-center px-6 py-4">
                      <span className="inline-flex items-center justify-center w-12 h-12 bg-daze-gray rounded-full font-semibold text-daze-black">
                        {entry.total_leads}
                      </span>
                    </td>
                    <td className="text-center px-6 py-4">
                      <span
                        className={`inline-flex items-center justify-center px-4 py-2 rounded-full text-sm font-semibold ${
                          entry.conversion_rate >= 70
                            ...
                            ? 'bg-daze-honey/10 text-daze-honey-dark'
